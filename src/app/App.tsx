...
export default function App() {
  ...
  const scrollToGroup = (groupId: string) => {
    ...
    if (element && contentRef.current) {
      ...
      // When header is visible: offset by full header height
      // When header is hidden: offset by sticky group height only (it sits at top:0)
      const offset = headerVisibleRef.current ? COLLAPSIBLE_HEADER_HEIGHT : GROUP_HEADER_HEIGHT;
      container.scrollTo({
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      ...
      <div
        className={`fixed top-0 left-0 right-0 bg-[#F6F7F9] transition-transform duration-300 ease-in-out ${isStatusDropdownOpen ? 'z-[52]' : 'z-40'}`}
        style={{
      ...
      {pinnedGroup && (
        <DroppableGroupHeader
          ...
          className={`fixed left-0 right-0 shadow-[0_4px_14px_rgba(13,45,94,0.08)] transition-[top,background-color] ${anyDrawerOpen ? 'z-[30]' : 'z-50'}`}
                ...
                <div>
                    {group.products.length === 0 && (
                      <div className="px-4 py-3">
                        <span className="text-[13px] text-[rgba(13,45,94,0.32)]">В группе нет товаров</span>
                      </div>
                    )}
